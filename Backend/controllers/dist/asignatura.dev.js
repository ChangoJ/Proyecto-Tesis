...
var controller = {
  save: function save(req, res) {
    ...
    if (validate_nombre && validate_horario && params.carrera.length !== 0 && params.semestre.length !== 0 && params.profesor.length !== 0 && validate_abreviatura && validate_color && params.creditos !== null && params.creditos !== undefined && Number.isInteger(params.creditos) && params.creditos !== 0 && params.creditos <= 20) {
      ...
      var asignatura1 = new asignatura(); //asignar valores
      ...
      asignatura1.creditos = params.creditos; //guardar el articulo
  ...
  saveForProfesor: function saveForProfesor(req, res) {
    // Recoger parametros por post
    var params = req.body; // Validar datos (validator)
    ...
    if (validate_nombre && validate_horario && params.carrera.length !== 0 && params.semestre.length !== 0 && params.profesor.length !== 0 && validate_abreviatura && validate_color && params.creditos !== null && params.creditos !== undefined && Number.isInteger(params.creditos) && params.creditos !== 0 && params.creditos <= 20) {
      var promises = params.profesor.map(function (profesor) {
      ...
      Promise.all(promises).then(function (asignaturasStored) {
  ...
  getAsignaturas: function getAsignaturas(req, res) {
    ...
    query.sort('-_id').then(function (asignaturas) {
      if (!asignaturas) {
        return res.status(404).send({
          status: 'error',
  ...
  getAsignatura: function getAsignatura(req, res) {
    //recoger el id de la url
    var asignaturaId = req.params.id;
    var asignaturaIdValid = mongoose.Types.ObjectId.isValid(asignaturaId); //comprobar que existe
    ...
    if (asignaturaIdValid) {
      if (!asignaturaId || asignatura == null) {
        ...
        asignatura.findById(asignaturaId).then(function (asignatura) {
          if (!asignatura) {
            return res.status(404).send({
              status: 'error',
              message: 'No existe la asignatura'
  ...
  update: function update(req, res) {
    ...
    var asignaturaId = req.params.id; //recoger datos del put
    ...
    if (asignaturaId.match(/^[0-9a-fA-F]{24}$/) && validate_nombre && validate_horario && params.carrera.length !== 0 && params.semestre.length !== 0 && params.profesor.length !== 0 && validate_abreviatura && validate_color) {
      asignatura.findOneAndUpdate({
        ...
        "new": true
      }).then(function (asignaturaUpdated) {
  ...
  "delete": function _delete(req, res) {
    var asignaturaId = req.params.id;
    asignatura.findByIdAndDelete({
      _id: asignaturaId
    }).then(function (asignaturaRemoved) {
  ...
  search: function search(req, res) {
    ...
    var search2Int = req.params.search2; //find and 
  ...
  searchThree: function searchThree(req, res) {
    //sacar strin a buscar
    var searchString = req.params.search1;
    var search2Int = req.params.search2;
    var search3String = req.params.search3; //find and 
  ...
  searchOne: function searchOne(req, res) {
    //sacar strin a buscar
